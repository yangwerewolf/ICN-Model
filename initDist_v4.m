function [ estm_state_n , p] = initDist_v4 ( thr, Lambda, lambda, init_state, transSto, e, s)
...
%     p = expcdf(stepN*lambda,Lambda);
    stepN = poisspdf(1:200,Lambda/lambda);
    temp = zeros(1,s+1);
    for i=1:200
        temp1 = stepN (i)* init_state(1:s+1)*(transNew^i);
        temp = temp + temp1;
    end
